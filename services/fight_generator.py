...
class FightGenerator:
    ...
    def generate_first_round_fights(
        self,
        athletes,
        round_number,
        start_fight_number,
    ...
    def generate_next_rounds_fights(
        self,
        next_fight,
        round_number,
        next_fight_number,
    ...
    def generate_consolation_fights_semifinalist(
        self, tournament_category_id, tatami_number, max_rounds
    ):
        semifinal_round = max_rounds - 1
        eight_round = semifinal_round - 1
        print(
            f"max_rounds={max_rounds}, semifinal_round={semifinal_round}, eight_round={eight_round}"
        )

        all_fights = self.fight_repo.get_untracked_semifinal_fights(
            tournament_category_id, semifinal_round, eight_round
        )
        ...
        return self._generate_semifinalist_fight(
            tournament_category_id, semifinal_round, eight_round, tatami_number
        )

    def generate_consolation_fights_finalist(
        self, tournament_category_id, tatami_number, max_rounds
    ):
        final_round = max_rounds
        eight_fight = max(1, max_rounds - 2)  # минимум 1
        return self._generate_finalist_fight(
            tournament_category_id, final_round, eight_fight, tatami_number
        )

    def _generate_group(
        self, all_fights: list[FightNew], semifinal_fight: FightNew
    ) -> list[FightNew]:
        """
    ...
    def _generate_fights_semifinalist_by_group(
        self,
        group_of_fights: list[FightNew],
        tournament_category_id: int,
        tatami_number: int,
        branch_name: BracketType,
    ) -> list[FightNew]:
        ...
        losers = [
            ...
            and loser.id != semifinal_loser_id
        ]

        if len(losers) == 0:
            raise Exception(
                f"❌ Нет участников для утешительной сетки в ветке {branch_name.name}"
            )
        ...
        first_fights = self.generate_first_round_fights(
            current_fighters,
            current_round,
            fight_number,
            tournament_category_id,
        ...
        if len(consolation_fights) == 1:
            ...
            color_loser_semifinalist = (
                "white"
                if semifinal_loser_id == semifinal_fight.white_athlete_id
                else "blue"
            )

            next_figth = FightNew(
                tournament_category_id=tournament_category_id,
                white_athlete_id=semifinal_loser_id
                if color_loser_semifinalist == "blue"
                else None,
                blue_athlete_id=semifinal_loser_id
                if color_loser_semifinalist == "white"
                else None,
    ...
    def _generate_semifinalist_fight(
        self, tournament_category_id, round, eight_round, tatami_number
    ) -> list[FightNew]:
        all_fights = self.fight_repo.get_untracked_semifinal_fights(
            tournament_category_id, round, eight_round
        )
        ...
        semifinal_fights = [
            fight for fight in all_fights if fight.round_number == round
        ]
        ...
        consolation_fights_a = self._generate_fights_semifinalist_by_group(
            group_a,
            tournament_category_id,
            tatami_number,
            BracketType.SEMIFINALIST_CONSOLATION_GROUP_A,
        )
        consolation_fights_b = self._generate_fights_semifinalist_by_group(
            group_b,
            tournament_category_id,
            tatami_number,
    ...
    def _generate_fights_finalist_by_group(
        self,
        group_of_fights: list[FightNew],
        tournament_category_id: int,
        tatami_number: int,
        ...
        if (
            first_fights[0].white_athlete_id is None
            or first_fights[0].blue_athlete_id is None
        ):
            self.fight_repo.update_status(first_fights[0].id, FightStatus.COMPLETED)

        return first_fights[0]

    def _generate_final_group(
        self, all_fights: list[FightNew], final_athlete_id: int, final_id: int
    ) -> list[FightNew]:
        group = []
        ...
        for fight in all_fights:
            ...
            if (
                fight_result
                and fight_result.winner_id == final_athlete_id
                and fight.id != final_id
            ):
                # Избегаем дубликатов
                if fight not in group:
                    group.append(fight)
    ...
    def _generate_finalist_fight(
        ...
        final_fights = [
            fight for fight in all_fights if fight.round_number == real_final_round
        ]
        ...
        fights_group_a = self._generate_fights_finalist_by_group(
            group_a,
            tournament_category_id,
            tatami_number,
            BracketType.FINALIST_CONSOLATION_GROUP_A,
        )
        fights_group_b = self._generate_fights_finalist_by_group(
            group_b,
            tournament_category_id,
            tatami_number,
